"""Freeze a candidate-union refinement plan from completed C1 k1 traces."""
...
from scripts.bonafide.build_topk_c1_bundle import C1_BUNDLE_SCHEMA
from scripts.bonafide.build_topk_manifest import save_manifest
from scripts.bonafide.execution_plan import sha256_file

REFERENCE_FAMILY_PREFIX = "bonafide.c1.independent-candidate-"
...
def _reference_index(reference_root: Path) -> dict[tuple[int, str], Path]:
    ...
    for candidate_index in range(6):
        ...
        if not family_root.is_dir():
            raise FileNotFoundError(f"missing C1 reference family: {family_root}")
        for manifest_path in family_root.glob("*/*/manifest.json"):
            manifest = _load_json(manifest_path)
            source_id = manifest.get("source_width1_artifact_id")
            if not isinstance(source_id, str) or not source_id:
                raise ValueError(f"invalid C1 reference source ID: {source_id!r}")
            key = (candidate_index, source_id)
            if key in result:
                raise ValueError(f"duplicate C1 reference: {key}")
            result[key] = manifest_path.parent.resolve()
    return result


def build_candidate_union_c1_plan(
    ...
    *,
    ...
    """Bind the frozen C1 cases to exact completed independent artifacts."""

    if bundle.get("schema_version") != C1_BUNDLE_SCHEMA:
        raise ValueError("unsupported C1 launch-bundle schema")
    cases = bundle.get("cases")
    if not isinstance(cases, list) or len(cases) != 32:
        raise ValueError("candidate-union C1 plan requires the frozen 32 cases")
    if candidate_zero_manifest.get("phase") != "c1_policy_resource":
        raise ValueError("candidate-zero manifest is not a C1 manifest")
    family = candidate_zero_manifest.get("trace_family", {})
    if family.get("trace_family_id") != "bonafide.c1.independent-candidate-0.v1":
        raise ValueError("candidate-zero C1 trace family drift")
    ...
    for case in cases:
        ...
        if (
            ...
            raise ValueError(f"invalid frozen C1 case: {case_id!r}")
        source_item = source_items.get(source_id)
        if source_item is None:
            raise ValueError(f"C1 case lacks its source work item: {source_id}")
        if source_item["target_selection"]["response_token_positions"] != [
            case["target_response_position"]
        ]:
            raise ValueError(f"C1 source position drift: {case_id}")
        ...
        for candidate_index, expected_token_id in enumerate(expected_tokens):
            ...
            if path is None:
                raise ValueError(
                    f"C1 case lacks candidate {candidate_index} reference: {case_id}"
                )
            ...
            if (
                ...
                raise ValueError(
                    f"C1 candidate reference contract drift: {case_id}, "
        ...
        if reference_paths.get((len(expected_tokens), source_id)) is not None:
            raise ValueError(f"C1 case has an unexpected extra reference: {case_id}")
        grouped[role].append(
            {
                "case_id": case_id,
    ...
    if {role: len(values) for role, values in grouped.items()} != {
        ...
        raise ValueError("candidate-union C1 role counts drifted")
    waves = []
    for role in ("dense_discovery", "broad_discovery"):
        for shard_index in range(2):
            shard = grouped[role][shard_index * 8 : (shard_index + 1) * 8]
            waves.append(
                {
                    "wave_id": (
                        f"candidate-union-c1-{role.replace('_discovery', '')}-"
                        f"{shard_index}"
                    ),
                    "corpus_role": role,
                    "cases": shard,
    ...
    return {
        ...
        "phase": "c1_candidate_union_refinement",
        "plan_id": "qwen3-4b-instruct-candidate-union-c1-v1",
        "source": {
...
def main() -> None:
    ...
    plan = build_candidate_union_c1_plan(
        bundle,
