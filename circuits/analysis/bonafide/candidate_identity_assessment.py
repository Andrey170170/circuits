...
def _hierarchical_weights(rows: Sequence[ProjectedRow]) -> NDArray[np.float64]:
...
def _load_source_rows(
    ...
    targets = [
        row for partition in PARTITIONS for row in target_tables[partition].to_pylist()
    ]
    ...
    *,
    ...
    dictionary_report = {
        view: {
            ...
            "canonical_keys": sorted(values, key=values.__getitem__),
        }
