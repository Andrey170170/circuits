...
def run_hybrid_candidate_clustering(
    *, input_root: Path, output_root: Path, repo_root: Path
    ...
    if code_revision["git_dirty"]:
        raise ValueError(
            "refuse to publish hybrid clustering from dirty tracked source"
        )
    ...
    try:
        ...
        for representation in REPRESENTATION_IDS:
            for affinity_mode in ("full_positive", "knn32"):
                ...
                fit = valid[0]
                if any(
                    (candidate.affinity != fit.affinity).nnz for candidate in valid[1:]
                ):
                    raise ValueError(
                        "hybrid affinity varies across cluster-count cells"
                    )
...
def load_hybrid_clustering_manifest(
    ...
    if repo_root is not None and collect_hybrid_code_revision(
        repo_root
    ...
    expected_assignment_count = (
        sum(fit["status"] == "valid" for fit in fits) * len(RANDOM_SEEDS) * basis_count
    )
