...
.token.structural{font-family:var(--ui);font-size:11px;color:#4f5a73;border:1px dashed #aeb6c8;background:#f4f6fa;padding:1px 5px}.token.component-overlap{color:#6e5c8f;border-color:#b8a9cc;background:#f5f0fa}.token.token-whitespace{color:#3d6175;border-color:#9bb9c7;background:#edf7fa}
...
function whitespaceGlyphs(value){return value.replace(/ /g,"·").replace(/\t/g,"⇥").replace(/\r/g,"␍").replace(/\n/g,"↵\n")}
...
function selectDraft(item,token){const existing=state.saved.get(token.key);state.draft={completionId:item.completionId,position:token.position,comment:existing?existing.comment:""};persist();renderReader()}
