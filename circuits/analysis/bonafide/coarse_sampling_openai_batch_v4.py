"""Receipt-bound native OpenAI Batch lifecycle for coarse qualification v4."""
...
RUN_SCHEMA = "adag.process-witness.coarse-openai-batch-run.v4"
SUBMISSION_INTENT_SCHEMA = (
    "adag.process-witness.coarse-openai-batch-submission-intent.v4"
)
SUBMISSION_SCHEMA = "adag.process-witness.coarse-openai-batch-submission.v4"
STATUS_SCHEMA = "adag.process-witness.coarse-openai-batch-status.v4"
EVENT_SCHEMA = "adag.process-witness.coarse-openai-batch-event.v4"
COLLECTION_SCHEMA = "adag.process-witness.coarse-openai-batch-collection.v4"
STAGE = "coarse_qualification_v4"
...
_BOUND_RUN_FILES = (
    ...
    "scripts/bonafide/process_witness_coarse_openai_batch_v4.py",
)
...
def _source_revision() -> dict[str, Any]:
    ...
    if Path(_git(root, "rev-parse", "--show-toplevel")) != root:
        raise ValueError("coarse v4 run repository root drift")
    if _git(root, "status", "--porcelain=v1", "--untracked-files=no"):
        raise ValueError("coarse v4 submission requires a clean tracked worktree")
    commit = _git(root, "rev-parse", "HEAD")
    files = []
    for relative in _BOUND_RUN_FILES:
        if _git(root, "ls-files", "--error-unmatch", "--", relative) != relative:
            raise ValueError(f"coarse v4 run source is untracked: {relative}")
        path = root / relative
        ...
        if file_sha256(path) != expected:
            raise ValueError(f"coarse v4 run source differs from HEAD: {relative}")
...
def _submit_openai_batch_v4(
    input_path: Path, *, run_id: str, stage: str, key_env: str
) -> dict[str, Any]:
    if key_env != "OPENAI_API_KEY":
        raise ValueError("coarse v4 API key environment drift")
    ...
    if getattr(batch, "input_file_id", None) != uploaded.id:
        raise ValueError("coarse v4 created Batch input file id drift")
    return _provider_batch_dict(batch)


def _retrieve_openai_batch_v4(batch_id: str, *, run_id: str) -> dict[str, Any]:
    batch = _openai_client().batches.retrieve(batch_id)
    return _provider_batch_dict(batch)


def _cancel_openai_batch_v4(batch_id: str, *, run_id: str) -> dict[str, Any]:
    batch = _openai_client().batches.cancel(batch_id)
    return _provider_batch_dict(batch)


def _discover_openai_batch_v4(*, run_id: str) -> dict[str, Any]:
    matches = []
    ...
    if len(matches) != 1:
        raise ValueError(
            f"coarse v4 recovery expected one metadata-matched Batch, found {len(matches)}"
        )
    return matches[0]
...
def _download_openai_batch_v4(
    ...
    if batch.id != batch_id or batch.status != "completed":
        raise ValueError("coarse v4 provider Batch is not collectable")
    ...
    if not raw_files:
        raise ValueError("coarse v4 completed Batch has no output or error file")
...
def _validate_provider_snapshot(
    ...
    *,
    ...
    if value.get("provider") != "openai":
        raise ValueError("coarse v4 provider receipt provider drift")
...
def submit_v4_batch(
    *,
    ...
    submitter: Callable[..., dict[str, Any]] = _submit_openai_batch_v4,
) -> dict[str, Any]:
    """Persist intent, upload exact JSONL, and create one Batch exactly once."""

    loaded = load_v4_qualification(qualification_root)
    projected = float(loaded["cost_plan"]["projected_upper_bound_usd"])
    if not 0 < maximum_authorized_cost_usd <= 20.0:
        raise ValueError("coarse v4 authorization must be in (0, 20]")
    if projected > maximum_authorized_cost_usd:
        raise ValueError("coarse v4 persisted cost ceiling exceeds authorization")
    source_revision = _source_revision()
    if run_root.exists():
        raise FileExistsError(f"coarse v4 run already exists: {run_root}")
    ...
    try:
        ...
        if file_sha256(input_path) != file_sha256(
            ...
            raise ValueError("coarse v4 run input copy drift")
        intent = _self_hashed(
            {
        ...
        if intent_path.is_file() and submission_intent_path.is_file():
            ...
            raise RuntimeError(
                "coarse v4 provider state is indeterminate; automatic retry is forbidden"
            ) from error
        raise
...
def _load_run_intent_bindings(
    ...
    if (
        ...
        raise ValueError("coarse v4 run intent contract drift")
    qualification = load_v4_qualification(Path(intent["qualification_root"]))
    current_source = _source_revision()
    if current_source["files"] != intent.get("source_revision", {}).get("files"):
        raise ValueError("coarse v4 executing source differs from submission source")
    if (
    ...
    if (
        ...
        raise ValueError("coarse v4 submission-intent binding drift")
...
def _load_run(run_root: Path) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    _verify_self_hash(submission, "submission_sha256", "coarse v4 submission")
    if (
        ...
        raise ValueError("coarse v4 submission run binding drift")
    ...
    if create_snapshot != provider:
        raise ValueError("coarse v4 provider create snapshot/submission drift")
    _validate_provider_snapshot(
        provider,
...
def recover_v4_submission(
    run_root: Path,
    discoverer: Callable[..., dict[str, Any]] = _discover_openai_batch_v4,
) -> dict[str, Any]:
    """Recover a receipt from the snapshot persisted immediately after create."""

    if (run_root / "submission.json").exists():
        raise FileExistsError("coarse v4 submission receipt already exists")
...
def cancel_v4_batch(
    *,
    run_root: Path,
    reason: str,
    canceler: Callable[..., dict[str, Any]] = _cancel_openai_batch_v4,
) -> dict[str, Any]:
    ...
    if not reason.strip():
        raise ValueError("coarse v4 cancellation requires a reason")
    if (run_root / "cancellation-intent.json").exists():
        raise FileExistsError("coarse v4 cancellation was already attempted")
    provider = submission["provider_response"]
    cancellation_intent = _self_hashed(
        {
            "schema_version": "adag.process-witness.coarse-openai-batch-cancellation-intent.v4",
            "recorded_at": _utc_now(),
            "run_intent_sha256": intent["run_intent_sha256"],
    ...
    try:
        ...
        receipt = _self_hashed(
            {
                "schema_version": "adag.process-witness.coarse-openai-batch-cancellation.v4",
                "recorded_at": _utc_now(),
        ...
        raise RuntimeError("coarse v4 cancellation state is indeterminate") from error


def check_v4_batch(
    *,
    run_root: Path,
    retriever: Callable[..., dict[str, Any]] = _retrieve_openai_batch_v4,
) -> dict[str, Any]:
    ...
    for index, path in enumerate(prior):
        if path.name != f"receipt-{index:04d}.json":
            raise ValueError("coarse v4 status receipt numbering drift")
        previous = _load_object(path)
        _verify_self_hash(previous, "status_sha256", "coarse v4 status")
        if (
            ...
            raise ValueError("coarse v4 status receipt chain drift")
...
def parse_v4_batch_row(
    ...
    if row.get("custom_id") != request["request_id"]:
        raise ValueError("coarse v4 Batch custom_id does not match request")
    common = {
...
def _jsonl_rows(content: bytes, source: str) -> list[dict[str, Any]]:
    try:
        ...
        raise ValueError(f"coarse v4 {source} file is not UTF-8") from error
    rows = []
    for line_number, line in enumerate(text.splitlines(), start=1):
        ...
        try:
            ...
            raise ValueError(
                f"coarse v4 {source} file has invalid JSON at line {line_number}"
            ) from error
        if not isinstance(value, dict):
            raise ValueError(f"coarse v4 {source} row is not an object")
        rows.append(value)
    return rows


def _estimate_v4_actual_cost(
...
def _load_or_create_collection_intent(
    *,
    ...
    if path.exists():
        ...
        _verify_self_hash(
            existing, "collection_intent_sha256", "coarse v4 collection intent"
        )
        if (
            ...
            raise ValueError("coarse v4 collection intent binding drift")
        return existing
    value = _self_hashed(
        {
...
def _write_or_verify_json(path: Path, value: Mapping[str, Any]) -> None:
    if path.exists():
        if _load_object(path) != value:
            raise ValueError(f"retained coarse v4 JSON artifact drift: {path}")
        return
    atomic_write_json(path, value)
...
def _write_or_verify_bytes(path: Path, value: bytes) -> None:
    if path.exists():
        if path.read_bytes() != value:
            raise ValueError(f"retained coarse v4 byte artifact drift: {path}")
        return
    atomic_write_bytes(path, value)


def collect_v4_batch(
    *,
    run_root: Path,
    downloader: Callable[..., tuple[dict[str, Any], dict[str, dict[str, Any]]]] = (
        _download_openai_batch_v4
    ),
) -> dict[str, Any]:
    ...
    if (run_root / "collection-manifest.json").exists():
        raise FileExistsError("coarse v4 Batch has already been collected")
    provider = submission["provider_response"]
    ...
    if snapshot.get("status") != "completed":
        raise ValueError("coarse v4 Batch is not completed")
    ...
    for source in ("output", "error"):
        ...
        if not isinstance(content, bytes):
            raise ValueError("coarse v4 downloaded provider content is not bytes")
        path = raw_root / f"{source}.jsonl"
        if item.get("file_id") != snapshot.get(f"{source}_file_id"):
            raise ValueError(f"coarse v4 {source} file id receipt drift")
        ...
        for row in _jsonl_rows(content, source):
            ...
            if not isinstance(request_id, str):
                raise ValueError("coarse v4 Batch row has no string custom_id")
            if request_id in rows_by_id:
                raise ValueError("coarse v4 Batch output/error union repeats custom_id")
    ...
    if unknown or missing:
        ...
        raise ValueError("coarse v4 Batch row union does not exactly cover requests")
    ...
    if file_sha256(price_path) != qualification["cost_plan"]["price_snapshot_sha256"]:
        raise ValueError("coarse v4 frozen price snapshot drift")
    ...
    for request in requests:
        event = parse_v4_batch_row(rows_by_id[request["request_id"]], request)
        usage = Usage.model_validate(event["usage"])
        cost, long_context_applied = _estimate_v4_actual_cost(
            prices,
    ...
    exact_unique_coverage = len(unique_arm_targets) == 24
    ...
    complete = _self_hashed(
        {
            ...
            "expected_unique_arm_target_coverage": 24,
