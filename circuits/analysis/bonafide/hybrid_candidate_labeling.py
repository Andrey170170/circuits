...
def _load_witness_inventory(root: Path) -> Mapping[str, Any]:
    ...
    if (
        ...
        or value.get("schema_version") != WITNESS_INVENTORY_SCHEMA
    ):
...
def _candidate_summary(
    *,
    ...
    width = len(axis)
    signed_sum = (
        np.sum(np.asarray(values), axis=0).tolist() if values else [0.0] * width
    )
    ...
    return {
        ...
        "signed_contribution_mean_per_occurrence": [
            value / occurrence_count if occurrence_count else 0.0
            for value in signed_sum
        ],
...
def _cluster_evidence(
    *,
    ...
    indices = np.asarray(
        [int(row["signed_basis_index"]) for row in members], dtype=np.int64
    )
    ...
    for partition in ("generation", "selection_scoring", "audit"):
        ...
        frozen_witnesses[partition] = {
            ...
            "ordered_target_ids_sha256": partition_record["frozen_target_ids_sha256"],
    ...
    *, evaluation_root: Path, input_root: Path, fit_root: Path, output_root: Path
    ...
    try:
        for role in STATE_ROLES:
            ...
            cluster_records = cast(
                Sequence[Mapping[str, Any]], inventory_state["clusters"]
            )
            if sorted(int(row["cluster_id"]) for row in cluster_records) != list(
                range(64)
            ):
...
def load_hybrid_labeling_bundle(root: Path) -> Mapping[str, Any]:
    ...
    if (
        manifest.get("purpose") != "provider_neutral_hybrid_candidate_labeling_evidence"
    ...
    repo_root = Path(__file__).resolve().parents[3]
    if not isinstance(revision, Mapping) or revision != collect_bridge_revision(
        repo_root
    ):
    ...
    evaluation = load_hybrid_candidate_labelability(
    ...
    evaluation_fit_binding = cast(Mapping[str, Any], evaluation["source_fit_binding"])
    ...
    for role in STATE_ROLES:
        ...
        if (
            ...
            is not (
                cast(Mapping[str, Any], reports[role]).get(
                    "exploratory_labeling_authorized"
                )
                is True
            )
            or state_manifest.get("ready_cluster_count") != len(state.ready_cluster_ids)
        ):
        ...
        if (
            inventory_state
            != _load_witness_inventory(Path(str(source_evaluation["path"])))["states"][
                role
            ]
        ):
        ...
        if (
            state_manifest.get("recommended_cluster_selection")
            != expected_recommendation
        ):
        ...
        for cluster_id, row in state.evidence.items():
            ...
            expected = (
                "ready"
                if authorized and source_ready
                else "state_or_cluster_not_authorized"
            )
