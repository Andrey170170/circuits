"""Receipt-bound native OpenAI Batch lifecycle for coarse qualification v3."""
...
RUN_SCHEMA = "adag.process-witness.coarse-openai-batch-run.v3"
SUBMISSION_INTENT_SCHEMA = (
    "adag.process-witness.coarse-openai-batch-submission-intent.v3"
)
SUBMISSION_SCHEMA = "adag.process-witness.coarse-openai-batch-submission.v3"
STATUS_SCHEMA = "adag.process-witness.coarse-openai-batch-status.v3"
EVENT_SCHEMA = "adag.process-witness.coarse-openai-batch-event.v3"
COLLECTION_SCHEMA = "adag.process-witness.coarse-openai-batch-collection.v3"
STAGE = "coarse_qualification_v3"

_BOUND_RUN_FILES = (
    "circuits/analysis/bonafide/coarse_sampling_openai_batch_v3.py",
    ...
    "scripts/bonafide/process_witness_coarse_openai_batch_v3.py",
)
...
def _source_revision() -> dict[str, Any]:
    ...
    if Path(_git(root, "rev-parse", "--show-toplevel")) != root:
        raise ValueError("coarse v3 run repository root drift")
    if _git(root, "status", "--porcelain=v1", "--untracked-files=no"):
        raise ValueError("coarse v3 submission requires a clean tracked worktree")
    commit = _git(root, "rev-parse", "HEAD")
    files = []
    for relative in _BOUND_RUN_FILES:
        if _git(root, "ls-files", "--error-unmatch", "--", relative) != relative:
            raise ValueError(f"coarse v3 run source is untracked: {relative}")
        path = root / relative
        ...
        if file_sha256(path) != expected:
            raise ValueError(f"coarse v3 run source differs from HEAD: {relative}")
...
def _submit_openai_batch_v3(
    input_path: Path, *, run_id: str, stage: str, key_env: str
) -> dict[str, Any]:
    if key_env != "OPENAI_API_KEY":
        raise ValueError("coarse v3 API key environment drift")
    ...
    if getattr(batch, "input_file_id", None) != uploaded.id:
        raise ValueError("coarse v3 created Batch input file id drift")
    return _provider_batch_dict(batch)


def _retrieve_openai_batch_v3(batch_id: str, *, run_id: str) -> dict[str, Any]:
    batch = _openai_client().batches.retrieve(batch_id)
    return _provider_batch_dict(batch)


def _cancel_openai_batch_v3(batch_id: str, *, run_id: str) -> dict[str, Any]:
    batch = _openai_client().batches.cancel(batch_id)
    return _provider_batch_dict(batch)


def _discover_openai_batch_v3(*, run_id: str) -> dict[str, Any]:
    matches = []
    for batch in _openai_client().batches.list(limit=100):
        value = _provider_batch_dict(batch)
        if value.get("metadata") == {"run_id": run_id, "stage": STAGE}:
            matches.append(value)
    if len(matches) != 1:
        raise ValueError(
            f"coarse v3 recovery expected one metadata-matched Batch, found {len(matches)}"
        )
    return matches[0]
...
def _download_openai_batch_v3(
    ...
    if batch.id != batch_id or batch.status != "completed":
        raise ValueError("coarse v3 provider Batch is not collectable")
    ...
    if not raw_files:
        raise ValueError("coarse v3 completed Batch has no output or error file")
...
def _validate_provider_snapshot(
    ...
    *,
    ...
    if value.get("provider") != "openai":
        raise ValueError("coarse v3 provider receipt provider drift")
...
def submit_v3_batch(
    *,
    ...
    submitter: Callable[..., dict[str, Any]] = _submit_openai_batch_v3,
) -> dict[str, Any]:
    """Persist intent, upload exact JSONL, and create one Batch exactly once."""

    loaded = load_v3_qualification(qualification_root)
    projected = float(loaded["cost_plan"]["projected_upper_bound_usd"])
    if not 0 < maximum_authorized_cost_usd <= 20.0:
        raise ValueError("coarse v3 authorization must be in (0, 20]")
    if projected > maximum_authorized_cost_usd:
        raise ValueError("coarse v3 persisted cost ceiling exceeds authorization")
    source_revision = _source_revision()
    if run_root.exists():
        raise FileExistsError(f"coarse v3 run already exists: {run_root}")
    ...
    try:
        ...
        if file_sha256(input_path) != file_sha256(
            ...
            raise ValueError("coarse v3 run input copy drift")
        intent = _self_hashed(
            {
        ...
        atomic_write_json(run_root / "provider-create-response.json", provider)
        ...
        if intent_path.is_file() and submission_intent_path.is_file():
            ...
            failure = _self_hashed(
                {
                    ...
                    "provider_create_response_path": (
                        "provider-create-response.json"
                        if (run_root / "provider-create-response.json").is_file()
                        else None
                    ),
                    "provider_create_response_sha256": (
                        file_sha256(run_root / "provider-create-response.json")
                        if (run_root / "provider-create-response.json").is_file()
                        else None
                    ),
            ...
            raise RuntimeError(
                "coarse v3 provider state is indeterminate; automatic retry is forbidden"
            ) from error
        raise


def _load_run_intent_bindings(
    run_root: Path,
) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    """Validate all local pre-provider identities under the executing source."""

    intent = _load_object(run_root / "run-intent.json")
    _verify_self_hash(intent, "run_intent_sha256", "coarse v3 run intent")
    if (
        intent.get("schema_version") != RUN_SCHEMA
        or intent.get("status") != "intent_persisted_before_provider_calls"
    ):
        raise ValueError("coarse v3 run intent contract drift")
    qualification = load_v3_qualification(Path(intent["qualification_root"]))
    current_source = _source_revision()
    if current_source["files"] != intent.get("source_revision", {}).get("files"):
        raise ValueError("coarse v3 executing source differs from submission source")
    if (
        ...
        raise ValueError("coarse v3 run/qualification binding drift")
    if intent["request_ids_in_order"] != [
        request["request_id"] for request in qualification["requests"]
    ]:
        raise ValueError("coarse v3 run request order drift")
    submission_intent = _load_object(run_root / "submission-intent.json")
    _verify_self_hash(
        submission_intent,
        "submission_intent_sha256",
        "coarse v3 submission intent",
    )
    if (
        submission_intent.get("schema_version") != SUBMISSION_INTENT_SCHEMA
        or submission_intent["run_intent_sha256"] != intent["run_intent_sha256"]
        ...
        or submission_intent.get("stage") != STAGE
    ):
        raise ValueError("coarse v3 submission-intent binding drift")
    return intent, qualification, submission_intent


def _load_run(run_root: Path) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    intent, qualification, submission_intent = _load_run_intent_bindings(run_root)
    submission = _load_object(run_root / "submission.json")
    _verify_self_hash(submission, "submission_sha256", "coarse v3 submission")
    if (
        submission.get("schema_version") != SUBMISSION_SCHEMA
        or submission["run_intent_sha256"] != intent["run_intent_sha256"]
        or submission["submission_intent_sha256"]
        != submission_intent["submission_intent_sha256"]
    ):
        raise ValueError("coarse v3 submission run binding drift")
    provider = submission["provider_response"]
    create_snapshot = _load_object(run_root / "provider-create-response.json")
    if create_snapshot != provider:
        raise ValueError("coarse v3 provider create snapshot/submission drift")
    _validate_provider_snapshot(
        provider,
...
def recover_v3_submission(
    run_root: Path,
    discoverer: Callable[..., dict[str, Any]] = _discover_openai_batch_v3,
) -> dict[str, Any]:
    """Recover a receipt from the snapshot persisted immediately after create."""

    if (run_root / "submission.json").exists():
        raise FileExistsError("coarse v3 submission receipt already exists")
    intent, _qualification, submission_intent = _load_run_intent_bindings(run_root)
    create_path = run_root / "provider-create-response.json"
    if create_path.exists():
        provider = _load_object(create_path)
        recovered_by = "immediate_create_snapshot"
    else:
        provider = discoverer(run_id=intent["run_intent_sha256"])
        atomic_write_json(create_path, provider)
        recovered_by = "unique_provider_metadata_discovery"
    _validate_provider_snapshot(
        provider,
        batch_id=provider.get("batch_id"),
...
def cancel_v3_batch(
    *,
    run_root: Path,
    reason: str,
    canceler: Callable[..., dict[str, Any]] = _cancel_openai_batch_v3,
) -> dict[str, Any]:
    """Persist cancellation intent before one exact provider cancellation call."""

    intent, _qualification, submission = _load_run(run_root)
    if not reason.strip():
        raise ValueError("coarse v3 cancellation requires a reason")
    if (run_root / "cancellation-intent.json").exists():
        raise FileExistsError("coarse v3 cancellation was already attempted")
    provider = submission["provider_response"]
    cancellation_intent = _self_hashed(
        {
            "schema_version": "adag.process-witness.coarse-openai-batch-cancellation-intent.v3",
            "recorded_at": _utc_now(),
            "run_intent_sha256": intent["run_intent_sha256"],
    ...
    atomic_write_json(run_root / "cancellation-intent.json", cancellation_intent)
    try:
        observed = canceler(provider["batch_id"], run_id=intent["run_intent_sha256"])
        _validate_provider_snapshot(
            observed,
            batch_id=provider["batch_id"],
            input_file_id=provider["input_file_id"],
        ...
        receipt = _self_hashed(
            {
                "schema_version": "adag.process-witness.coarse-openai-batch-cancellation.v3",
                "recorded_at": _utc_now(),
                "cancellation_intent_sha256": cancellation_intent[
                    "cancellation_intent_sha256"
                ],
        ...
        atomic_write_json(run_root / "cancellation.json", receipt)
        return receipt
    except BaseException as error:
        failure = _self_hashed(
            {
                "schema_version": "adag.process-witness.coarse-openai-batch-cancellation.v3",
                "status": "failed_closed_indeterminate_cancellation_state",
                "recorded_at": _utc_now(),
                "cancellation_intent_sha256": cancellation_intent[
                    "cancellation_intent_sha256"
                ],
            ...
            "cancellation_failure_sha256",
        )
        atomic_write_json(run_root / "cancellation-failure.json", failure)
        raise RuntimeError("coarse v3 cancellation state is indeterminate") from error


def check_v3_batch(
    *,
    run_root: Path,
    retriever: Callable[..., dict[str, Any]] = _retrieve_openai_batch_v3,
) -> dict[str, Any]:
    ...
    for index, path in enumerate(prior):
        if path.name != f"receipt-{index:04d}.json":
            raise ValueError("coarse v3 status receipt numbering drift")
        previous = _load_object(path)
        _verify_self_hash(previous, "status_sha256", "coarse v3 status")
        if (
            ...
            raise ValueError("coarse v3 status receipt chain drift")
...
def parse_v3_batch_row(
    ...
    if row.get("custom_id") != request["request_id"]:
        raise ValueError("coarse v3 Batch custom_id does not match request")
    common = {
...
def _jsonl_rows(content: bytes, source: str) -> list[dict[str, Any]]:
    try:
        ...
        raise ValueError(f"coarse v3 {source} file is not UTF-8") from error
    rows = []
    for line_number, line in enumerate(text.splitlines(), start=1):
        ...
        try:
            ...
            raise ValueError(
                f"coarse v3 {source} file has invalid JSON at line {line_number}"
            ) from error
        if not isinstance(value, dict):
            raise ValueError(f"coarse v3 {source} row is not an object")
        rows.append(value)
    return rows


def _estimate_v3_actual_cost(
...
def _load_or_create_collection_intent(
    *,
    ...
    if path.exists():
        ...
        _verify_self_hash(
            existing, "collection_intent_sha256", "coarse v3 collection intent"
        )
        if (
            ...
            raise ValueError("coarse v3 collection intent binding drift")
        return existing
    value = _self_hashed(
        {
...
def _write_or_verify_json(path: Path, value: Mapping[str, Any]) -> None:
    if path.exists():
        if _load_object(path) != value:
            raise ValueError(f"retained coarse v3 JSON artifact drift: {path}")
        return
    atomic_write_json(path, value)
...
def _write_or_verify_bytes(path: Path, value: bytes) -> None:
    if path.exists():
        if path.read_bytes() != value:
            raise ValueError(f"retained coarse v3 byte artifact drift: {path}")
        return
    atomic_write_bytes(path, value)


def collect_v3_batch(
    *,
    run_root: Path,
    downloader: Callable[..., tuple[dict[str, Any], dict[str, dict[str, Any]]]] = (
        _download_openai_batch_v3
    ),
) -> dict[str, Any]:
    ...
    if (run_root / "collection-manifest.json").exists():
        raise FileExistsError("coarse v3 Batch has already been collected")
    provider = submission["provider_response"]
    ...
    if snapshot.get("status") != "completed":
        raise ValueError("coarse v3 Batch is not completed")
    ...
    for source in ("output", "error"):
        ...
        if not isinstance(content, bytes):
            raise ValueError("coarse v3 downloaded provider content is not bytes")
        path = raw_root / f"{source}.jsonl"
        if item.get("file_id") != snapshot.get(f"{source}_file_id"):
            raise ValueError(f"coarse v3 {source} file id receipt drift")
        ...
        for row in _jsonl_rows(content, source):
            ...
            if not isinstance(request_id, str):
                raise ValueError("coarse v3 Batch row has no string custom_id")
            if request_id in rows_by_id:
                raise ValueError("coarse v3 Batch output/error union repeats custom_id")
    ...
    if unknown or missing:
        ...
        raise ValueError("coarse v3 Batch row union does not exactly cover requests")

    price_path = (
        Path(intent["qualification_root"])
        / qualification["cost_plan"]["price_snapshot_relative_path"]
    )
    if file_sha256(price_path) != qualification["cost_plan"]["price_snapshot_sha256"]:
    ...
    for request in requests:
        event = parse_v3_batch_row(rows_by_id[request["request_id"]], request)
        usage = Usage.model_validate(event["usage"])
        cost, long_context_applied = _estimate_v3_actual_cost(
            prices,
    ...
    provider_request_ids = [event.get("provider_request_id") for event in success]
    distinct_provider_responses = (
        len(provider_request_ids) == len(requests)
        and all(isinstance(value, str) and value for value in provider_request_ids)
        and len(set(provider_request_ids)) == len(provider_request_ids)
    )
    unique_arm_targets = {
    ...
    complete = _self_hashed(
        {
            ...
            "distinct_provider_response_identity": distinct_provider_responses,
            "unique_provider_request_ids": len(set(provider_request_ids)),
            "unique_arm_target_coverage": len(unique_arm_targets),
            "expected_unique_arm_target_coverage": 288,
