...
def _mapped_occurrence(
    ...
    return {
        ...
        "paper_candidate_values": [
            float(value) for value in row["paper_normalized_candidate_contribution"]
        ],
...
def extract_evaluation_occurrences(
    ...
    occurrences = [
        _mapped_occurrence(
            row, target_by_case[str(row["case_id"])], int(row["signed_basis_index"])
        )
    ...
    return (
        occurrences,
        [target_by_case[key] for key in sorted(target_by_case)],
        {
...
def _target_vectors(
    ...
    *,
    ...
    paper = np.asarray(
        [row["paper_candidate_values"] for row in rows], dtype=np.float32
    )
    representation: Representation = (
        "contrast"
        if view == "contrast"
        else cast(Representation, spec["representation_key"])
    )
...
def _bootstrap(
    ...
    *,
    ...
    draws = (
        ...
        .choice(ordered, size=(BOOTSTRAP_REPLICATES, len(ordered)), replace=True)
        .mean(axis=1)
    )
...
def evaluate_coherence(
    ...
    *,
    ...
    for target_id in sorted(by_target):
        ...
        for view in VIEWS:
            effects = _target_effects(
                by_target[target_id], target, assignments, view=view
            )
    ...
    for role, spec in STATE_SPECS.items():
        ...
        for cluster_id in range(64):
            ...
            for partition, (
                ...
                ready = (
                    len(target_ids) >= target_minimum
                    and len(family_ids) >= family_minimum
                )
                ...
                if ready:
                    ...
                    for family in family_order:
                        choices = [
                            ...
                            if target_to_family[target] == family
                        ]
                        frozen_ids.append(
                            min(
                                choices,
                    ...
                    remaining = [
                        target for target in target_ids if target not in frozen_ids
                    ]
                    remaining.sort(
                        key=lambda target: hashlib.sha256(
                            f"{protocol_sha256}\0{role}\0{cluster_id}\0{partition}\0{target}".encode()
    ...
    for role, spec in STATE_SPECS.items():
        ...
        observed = {
            "assignment_fraction": float(
                fit["assignment_fraction_by_seed"][str(spec["seed"])]
            ),
            "maximum_cluster_fraction": float(
                fit["cluster_size_metrics"]["maximum_cluster_fraction"]
            ),
        ...
        conditions = {
            ...
            "internal_affinity_enrichment": observed["internal_affinity_enrichment"]
            >= 1.25,
        }
        output[role] = {
            "observed": observed,
...
def _jackknife_evidence(
    ...
    eligible = (
        ...
        & (
            np.asarray(pair.response_overlap_count.diagonal()).ravel()
            >= MIN_BASIS_RESPONSES
        )
        & (
            np.asarray(pair.family_overlap_count.diagonal()).ravel()
            >= MIN_BASIS_FAMILIES
        )
    )
...
def generation_family_jackknife(
    ...
    for role, spec in STATE_SPECS.items():
        ...
        for family in families:
            try:
                ...
                if common_fraction < 0.80:
                    raise ValueError(
                        "jackknife common-assignment coverage is below 80%"
                    )
                ari = float(
                    adjusted_rand_score(labels[common], assignments[role][common])
                )
                if not np.isfinite(ari) or fit.medoid_seed not in (17, 29, 43):
                    raise ValueError(
                        "jackknife fit produced a nonfinite or invalid result"
                    )
                replicates.append(
                    {
        ...
        p10_ari = (
            None if not values else float(np.percentile(values, 10, method="linear"))
        )
        passed = (
            len(values) == 18
    ...
    for role in STATE_SPECS:
        ...
        for partition in HELDOUT_PARTITIONS:
            for view in ("input", "candidate"):
                ...
                conditions[f"{partition}_{view}_all_families_scoreable"] = (
                    len(effects) == 8
                )
                ...
                conditions[f"{partition}_{view}_seven_family_effects_positive"] = (
                    sum(float(value) > 0 for value in effects.values()) >= 7
                )
...
def _payload_binding(targets: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "case_set_sha256": canonical_sha256(
            sorted(str(row["case_id"]) for row in targets)
        ),
    ...
    *,
    ...
    if output_root.exists():
        raise FileExistsError(
            f"hybrid labelability destination already exists: {output_root}"
        )
    ...
    try:
        ...
        manifest: dict[str, Any] = {
            ...
            "signed_basis_mapping": {
                ...
                "generation_basis_index_sha256": canonical_sha256(
                    list(bundle.basis_rows)
                ),
                **extraction,
            },
            "coherence": coherence,
...
def load_hybrid_candidate_labelability(
    ...
    if manifest.get(
        "schema_version"
    ) != EVALUATION_SCHEMA or recorded != canonical_sha256(core):
    ...
    source_hashes = {row["path"]: row["sha256"] for row in revision.get("files", [])}
    if set(source_hashes) != set(EVALUATION_SOURCE_PATHS) or manifest.get(
        "protocol"
    ) != {
        "path": PROTOCOL_PATH,
    ...
    if (
        targets_payload.get("schema_version") != TARGET_SCHEMA
    ...
    _validate_occurrences(occurrence_rows, targets, basis_count=len(bundle.basis_rows))
