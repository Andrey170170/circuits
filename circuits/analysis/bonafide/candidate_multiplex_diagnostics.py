...
def _unit(value: object) -> NDArray[Any] | None:
...
def _mean_vectors(vectors: Iterable[NDArray[Any]]) -> NDArray[Any] | None:
...
def _hierarchical_vector(
    ...
    by_target: dict[tuple[str, str, str], list[NDArray[Any]]] = defaultdict(list)
    for row in rows:
    ...
    target_values: dict[tuple[str, str, str], NDArray[Any]] = {}
    ...
    by_response: dict[tuple[str, str], list[NDArray[Any]]] = defaultdict(list)
    ...
    by_family: dict[str, list[NDArray[Any]]] = defaultdict(list)
...
def _hierarchical_rank_mass(rows: Sequence[Mapping[str, Any]]) -> NDArray[Any]:
...
def _separation(rows: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    for key, target_rows in by_target.items():
        ...
        for left in range(len(units)):
            for right in range(left + 1, len(units)):
                left_unit = cast(NDArray[Any], units[left][0])
                right_unit = cast(NDArray[Any], units[right][0])
                cosine = float(np.dot(left_unit, right_unit))
    ...
    response_values: dict[tuple[str, str], tuple[float, float, float]] = {
        key: (
            float(mean(item[0] for item in values)),
            float(mean(item[1] for item in values)),
            float(mean(item[2] for item in values)),
        )
        for key, values in by_response.items()
    }
