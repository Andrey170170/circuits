...
def paper_normalize_occurrence(
    ...
    *,
    ...
    normalized_attr = [
        None
        if value is None
        else _finite(value, "attr_map") / activation_denominator
...
def _validate_profile_rows(
    ...
    *,
    ...
    for row in rows:
        ...
        if any(row[field] != basis_rows[index][field] for field in _BASIS_IDENTITY_FIELDS):
...
def load_hybrid_input_bundle(root: Path) -> HybridInputBundle:
    ...
    if not isinstance(representation_contract, Mapping) or (
        ...
        != {
            ...
            "small_denominator_fallback": 1.0,
            "aggregation_order": (
                "normalize_per_occurrence_then_sum_by_signed_basis"
            ),
        }
    ):
    ...
    if (
        source.manifest.get("manifest_sha256")
        != source_record.get("manifest_sha256")
        or source.manifest.get("schema_version")
    ...
    identities = [tuple(row[field] for field in _BASIS_IDENTITY_FIELDS) for row in basis_rows]
