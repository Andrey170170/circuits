...
def build_review_payload(
    *,
    ...
    if set(packet_identity["tag_definitions"]) != set(COARSE_TAGS):
        raise ValueError("coarse v3 review tag definitions drift")
    if set(packet_identity["boundary_definitions"]) != set(BOUNDARY_CONCERNS):
        raise ValueError("coarse v3 review boundary definitions drift")
    if len(packet_identity["decision_precedence"]) != len(COARSE_TAGS) or set(
        packet_identity["decision_precedence"]
    ) != set(COARSE_TAGS):
        raise ValueError("coarse v3 review decision precedence drift")
...
<aside class="card side" id="blind"><h3>Blind human judgment</h3><p class="meta">Model outputs are absent from this page. The page is populated automatically; Import progress is only for resuming an earlier export. Save all 144 judgments before the global seal.</p><label>Primary label<select id="primary"><option value="">Choose…</option></select></label><fieldset class="checks"><legend>Optional defensible alternatives</legend><div id="alternatives"></div></fieldset><fieldset class="checks"><legend>Boundary concerns</legend><div id="boundaries"></div></fieldset><label>Note<textarea id="note" placeholder="Optional rationale or ambiguity note"></textarea></label><div class="actions"><button id="save">Save item</button><button id="saveNext">Save &amp; next</button></div><p id="saveStatus" class="meta"></p><hr><p><strong>Global seal:</strong> irreversible after all 144 primary labels are saved.</p><button id="seal" disabled>Seal all 144 blind decisions</button><p class="warn" id="sealStatus"></p><p class="claim">These are coarse trace-sampling labels, not correctness, faithfulness, motif, or computation judgments.</p></aside>
...
const key=`coarse-review-v3:${{packet.packet_id}}:${{packet.ui_version}}`;let state={{index:0,sealed:false,sealed_at:null,decisions:{{}}}},visible=[...DATA.items.keys()];
...
function decisionsValid(s){{if(!s||!Number.isInteger(s.index)||s.index<0||s.index>=144||typeof s.decisions!=='object')return false;for(const [id,d] of Object.entries(s.decisions)){{const item=DATA.items.find(x=>x.item_id===id);if(!item||!validDecision(d,item))return false}}return true}}
function completeDecisions(s){{return decisionsValid(s)&&Object.keys(s.decisions).length===144&&DATA.items.every(item=>validDecision(s.decisions[item.item_id],item))}}
