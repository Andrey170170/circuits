...
import circuits.analysis.bonafide.coarse_sampling_openai_batch_v3 as module
import pytest
...
def _request(window: int, arm: str, replica: int) -> dict:
    ...
    return {
        ...
        "focal_unit_ids": [f"unit-{window}-{offset}" for offset in range(6)],
        "provider_body": body,
    }
...
def test_parser_preserves_arm_replica_and_exact_decisions() -> None:
    request = _request(3, ARM_FEW_SHOT, 2)
    event = parse_v3_batch_row(_row(request), request)
    assert event["validation_status"] == "success"
    assert event["arm_id"] == ARM_FEW_SHOT
...
def test_collect_requires_all_144_requests_and_288_unique_arm_targets(
    ...
    def download(*args, **kwargs):
        return {
            "status": "completed",
            "output_file_id": "output-v3",
            "error_file_id": None,
        }, {"output": {"file_id": "output-v3", "content": content}}

    run = tmp_path / "run"
    run.mkdir()
    manifest = collect_v3_batch(run_root=run, downloader=download)
    assert manifest["status"] == "complete"
    assert manifest["success_count"] == 144
    assert manifest["unique_arm_target_coverage"] == 288
