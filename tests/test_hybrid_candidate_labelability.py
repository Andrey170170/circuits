...
def test_common_pair_pool_is_intersected_before_state_split() -> None:
    ...
    rows = [_occurrence("case", "family", "audit", index) for index in range(4)]
    assignments = {
...
def test_readiness_requires_joint_same_occurrence_and_freezes_8_4_4() -> None:
    ...
    assert (
        len(cluster["joint_witnesses"]["selection_scoring"]["frozen_target_ids"]) == 4
    )
    ...
    assert (
        reduced["states"]["primary"]["clusters"][0]["joint_witnesses"]["generation"][
            "target_count"
        ]
        == 7
    )
