...
def test_v4_config_and_request_freeze_two_targets_three_replicas() -> None:
    ...
    assert config["qualification"]["focal_units_per_window"] == 2
    assert decision_json_schema_v4()["properties"]["decisions"]["minItems"] == 2
    ...
    window = {
        ...
        "focal_unit_ids": [u["unit_id"] for u in units[:FOCAL_UNITS_PER_WINDOW]],
    }
    primary = _request(
        physical_index=0,
        replica_index=0,
    ...
    assert schema == decision_json_schema_v4()
    json.dumps(primary)
...
def test_v4_blind_review_has_24_items_and_no_model_output() -> None:
    ...
    for index in range(12):
        document = _document(f"Repair sentence {index}. Control sentence {index}.")
    ...
    assert payload["packet"]["counts"] == {"response_blocks": 12, "items": 24}
