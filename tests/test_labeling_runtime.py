...
def test_hybrid_cluster_limit_is_twelve_per_authorized_state() -> None:
    bundle = SimpleNamespace(
        states={
            role: SimpleNamespace(manifest={"exploratory_labeling_authorized": True})
            for role in ("primary", "alternative")
        }
    )
    assert hybrid_cluster_limits(bundle, ["primary", "alternative"], None, 12) == {
        "primary": 12,
        "alternative": 12,
    }
    ...
    with pytest.raises(ValueError, match="overrides are forbidden"):
        hybrid_cluster_limits(bundle, ["primary", "alternative"], {"primary": [1]}, 12)
